...
- at least one of `target_start_time_utc_seconds` and `target_initial_receive_amount` must be `None`
- if `target_start_time_utc_seconds` is `Some`, it must be set to some timestamp in the future
- if `target_initial_receive_amount` is `Some`, it must be greater than or equal to `minimum_receive_amount`
