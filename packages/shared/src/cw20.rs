...
use cosmwasm_std::{
    to_json_binary, Addr, Api, BankMsg, Binary, Coin, CosmosMsg, Deps, MessageInfo, StdResult,
    WasmMsg,
};
...
#[cfg(not(test))]
pub fn is_cw20_token(_api: &dyn Api, denom: &str) -> StdResult<Addr> {
    if denom.len() > 20 {
        Ok(Addr::unchecked(denom))
    } else {
...
pub fn into_bank_msg(api: &dyn Api, recipient: Addr, amount: Vec<Coin>) -> StdResult<CosmosMsg> {
    ...
    Ok(match is_cw20_token(api, token.denom.as_ref()) {
        Ok(token_address) => CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_json_binary(&Cw20ExecuteMsg::Transfer {
                recipient: api.addr_validate(recipient.as_ref())?.to_string(),
                amount: token.amount,
            })?,
            funds: vec![],
        }),
